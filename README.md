...
- **连续处理**: 支持连续转换多个视频，Whisper 模式自动沿用
- **错误重试**: 失败时自动询问是否重试（最多 3 次）
...
1. **选择语音转文字方式** - 选择 Whisper API 或本地模型
2. **输入视频链接** - 输入 B站/YouTube 等视频链接
3. **自动执行转换** - 6 个阶段自动完成
4. **转换完成** - 查看任务目录中的结果
...
转换成功后，可选择继续处理下一个视频：
- 输入 `n` 并回车 → 退出程序
- 输入其他内容或直接回车 → 继续处理下一个视频

选择本地 Whisper 模式时，模型只在首次使用时加载，后续视频会复用已加载的模型。
